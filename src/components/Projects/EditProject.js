...
/* StaffDropdown component remains the same */
const StaffDropdown = ({ role, staffList, addMember }) => {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const ref = useRef(null);
    ...
    <div ref={ref} className="relative">
      <div className="team-row">
        ...
        <button
          type="button"
          onClick={() => { if (query.trim()) { addMember(role, query.trim()); setQuery(""); setOpen(false); } }}
          className="h-10 sm:h-11 px-3 sm:px-4 bg-teal-600 hover:bg-teal-700 active:bg-teal-800 text-white rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 transition-colors shadow-sm"
          title={`Add ${role}`}
        >
          <Plus size={18} />
        </button>
      ...
      {open && (
        <div className="absolute z-50 left-0 right-10 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden max-h-52 overflow-y-auto">
          {filtered.length === 0 && (
            <div className="px-4 py-3 text-xs text-slate-400 italic text-center">
              {query.trim()
                ? <span>Press <kbd className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 not-italic font-mono text-[11px]">Enter</kbd> to add "<b className="text-teal-600 not-italic">{query}</b>"</span>
          ...
          {filtered.map((s) => (
            <button
              ...
              type="button"
              onMouseDown={(e) => { e.preventDefault(); pick(s.employeeName); }}
              className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-teal-50 text-left transition-colors border-b border-slate-100 last:border-0"
            >
              <div className="w-7 h-7 rounded-full bg-teal-100 flex items-center justify-center shrink-0">
                <User size={13} className="text-teal-600" />
              </div>
          ...
          {filtered.length > 0 && query.trim() && !filtered.some(s => s.employeeName.toLowerCase() === query.toLowerCase()) && (
            <button
              type="button"
              onMouseDown={(e) => { e.preventDefault(); addMember(role, query.trim()); setQuery(""); setOpen(false); }}
              className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-amber-50 text-left transition-colors border-t border-slate-100"
            >
              <div className="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center shrink-0">
                <Plus size={13} className="text-amber-600" />
              </div>
...
const EditProject = () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({
            ...
            milestone: project.milestone != null ? String(project.milestone) : "",
            status: project.status || "active",
          });

          const rawTeam = project.teamMembers || {};
          setTeamMembers({
            frontend: Array.isArray(rawTeam.frontend) ? rawTeam.frontend : [],
            backend: Array.isArray(rawTeam.backend) ? rawTeam.backend : [],
            designer: Array.isArray(rawTeam.designer) ? rawTeam.designer : [],
            tester: Array.isArray(rawTeam.tester) ? rawTeam.tester : [],
            manager: Array.isArray(rawTeam.manager) ? rawTeam.manager : [],
          });

          setMilestonePayments(Array.isArray(project.milestonePayments) ? project.milestonePayments : []);
        } else {
          showToast("error", data.message || "Failed to load project");
        }
      } catch (error) {
        console.error("Fetch error:", error);
        showToast("error", "Network error: " + (error.message || "unknown"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const addMember = useCallback((role, name) => {
    if (!name || !name.trim()) return;
    setTeamMembers(prev => {
      if (prev[role].includes(name.trim())) return prev;
      return { ...prev, [role]: [...prev[role], name.trim()] };
    });
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Remove file fields if your backend doesn't have them
        // uploadfile: null,
        // quotationfile: null
      };

      console.log("📤 Updating project with payload:", JSON.stringify(payload, null, 2));
    ...
    <div className="min-h-screen bg-slate-50">
      <style>{`
        ...
        *, *::before, *::after { font-family: 'Inter', -apple-system, sans-serif; box-sizing: border-box; }
        .pg-card { background: #fff; border: 1px solid rgba(20,184,166,0.13); box-shadow: 0 2px 16px rgba(0,0,0,0.05); }
        .e-inp { width: 100%; background: #fff; border: 1.5px solid #e2e8f0; transition: border-color .2s, box-shadow .2s; }
        .e-inp:focus-within { border-color: #0d9488; box-shadow: 0 0 0 3px rgba(13,148,136,0.11); outline: none; }
        .e-inp input:focus { outline: none; }
        .team-row { display: flex; gap: 8px; }
        .team-row > div:first-child { flex: 1; min-width: 0; }
        .s-div { display: flex; align-items: center; gap: 10px; margin-bottom: 1rem; }
        .s-div::before, .s-div::after { content: ''; flex: 1; height: 1px; background: linear-gradient(to right, transparent, rgba(20,184,166,.25), transparent); }
      ...
      <div className="w-full max-w-4xl mx-auto px-3 sm:px-5 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-7 fade-up">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => navigate("/projects")}
              className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white shadow border border-gray-100 flex items-center justify-center text-gray-500 hover:text-teal-600 hover:border-teal-200 transition-all shrink-0">
              ←
            </button>
            <div className="min-w-0">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-800 tracking-tight">Edit Project</h1>
              <p className="text-xs text-slate-400 mt-0.5">Update project details &amp; team</p>
            </div>
          </div>
          <span className={`self-start sm:self-auto shrink-0 px-3 py-1.5 rounded-full text-xs font-bold capitalize ${
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 fade-up">
          ...
          <div className="pg-card rounded-2xl sm:rounded-3xl p-4 sm:p-6">
            <div className="s-div">
              <span className="text-xs font-bold text-teal-600 tracking-widest uppercase whitespace-nowrap">📋 Project Information</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
              <EInp name="projectId" value={formData.projectId} onChange={handleChange} label="Project ID" placeholder="Project ID" disabled />
              <EInp name="projectName" value={formData.projectName} onChange={handleChange} label="Project Name" placeholder="Project Name" required />
              <EInp name="clientName" value={formData.clientName} onChange={handleChange} label="Client Name" placeholder="Client Name" required />
              <EInp name="clientMobile" value={formData.clientMobile} onChange={handleChange} label="Client Mobile" placeholder="Mobile Number" />
              <EInp name="clientEmail" value={formData.clientEmail} onChange={handleChange} label="Client Email" placeholder="Email Address" type="email" />
          ...
          <div className="pg-card rounded-2xl sm:rounded-3xl p-4 sm:p-6">
            <div className="s-div">
              <span className="text-xs font-bold text-teal-600 tracking-widest uppercase whitespace-nowrap">📅 Schedule &amp; Budget</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
              <EInp name="startDate" value={formData.startDate} onChange={handleChange} label="Start Date" type="date" />
              <EInp name="projectHandoverDate" value={formData.projectHandoverDate} onChange={handleChange} label="Handover Date" type="date" />
          ...
          <div className="pg-card rounded-2xl sm:rounded-3xl p-4 sm:p-6">
            <div className="s-div">
              <span className="text-xs font-bold text-teal-600 tracking-widest uppercase whitespace-nowrap">👥 Team Members</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              {ROLES.map((role) => (
                <div key={role}>
                  ...
                  <StaffDropdown role={role} staffList={staffList} addMember={addMember} />

                  {teamMembers[role].length > 0 ? (
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {teamMembers[role].map((m, i) => (
                        <span key={`${role}-${i}`}
                          className="flex items-center gap-1.5 bg-teal-50 border border-teal-200 text-teal-800 px-2.5 py-1 rounded-full text-xs font-semibold">
                          {m}
                          <button type="button" onClick={() => removeMember(role, i)}
                            className="text-teal-500 hover:text-red-500 transition-colors leading-none">
                            <X size={11} />
                          </button>
                        </span>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 pt-1">
            <button type="button" onClick={() => navigate("/projects")}
              className="flex-1 sm:flex-none sm:w-32 py-3 rounded-xl border-2 border-gray-200 font-bold text-sm text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-all">
              Cancel
            </button>
            <button type="submit" disabled={submitting}
              ...
              {submitting ? <><Loader size={17} className="spin-i" /> Updating…</> : "Update Project"}
            </button>
          </div>
        </form>
      </div>
...
const EInp = ({ label, className = "", ...props }) => (
  <div>
    {label && <label className="text-xs sm:text-sm font-semibold text-slate-700 mb-1 sm:mb-1.5 block">{label}</label>}
    <input {...props} className={`e-inp h-10 sm:h-11 px-3.5 rounded-lg sm:rounded-xl text-sm text-slate-700 ${className}`} />
  </div>
);
