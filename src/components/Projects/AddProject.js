...
const AddProject = () => {
  ...
  const handleTimeline = (e) =>
    setAppDetails({
      ...appDetails,
      timeline: { ...appDetails.timeline, [e.target.name]: e.target.value },
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add appDetails only for non-marketing projects
      ...
      // Add marketing details only for digital market projects
      if (projectType === "digital market") {
        payload.marketing = {
          reels: {
      ...
      if (response.ok && data.success !== false) {
        showToast("success", `Project "${data.data.projectName}" created successfully!`);
        setTimeout(() => navigate("/projects"), 1800);
      } else {
        const msg = data.errors
          ? data.errors.join(", ")
