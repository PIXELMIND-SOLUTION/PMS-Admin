...
const Header = ({ onToggleSidebar, onLogout, isMobile, isTablet, sidebarOpen }) => {
  ...
  return (
    <>
      ...
      <header className={`sticky top-0 px-3 md:px-4 lg:px-5 py-2 md:py-3 flex justify-between items-center transition-all duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1)] z-40 ${
        isScrolled 
          ? 'bg-white/85 backdrop-blur-xl border-b border-teal-400/20 shadow-md shadow-teal-900/10' 
          : 'bg-white/95 backdrop-blur-xl border-b border-white/30 shadow-sm'
      }`}>
        <div className="flex items-center gap-2 md:gap-3">
          {/* Premium Menu Toggle Button */}
          <button
            className="bg-teal-700/10 border border-teal-700/20 rounded-xl w-11 h-11 flex items-center justify-center text-teal-700 transition-all duration-300 hover:bg-teal-700/20 hover:border-teal-700/40 hover:scale-105 shadow-md shadow-teal-700/20"
            onClick={onToggleSidebar}
            aria-label="Toggle menu"
          >
            {sidebarOpen && !isMobile ? <MdClose size={22} /> : <MdMenu size={22} />}
          ...
          <div className="flex flex-col">
            <h2 className={`mb-0 font-bold bg-gradient-to-r from-teal-700 to-teal-800 bg-clip-text text-transparent tracking-tight ${
              isMobile ? 'text-lg' : 'text-2xl'
            }`}>
              {isMobile ? 'Dashboard' : 'PixelmindSolutions'}
            </h2>
            <span className="hidden md:block text-gray-500 text-[11px] tracking-wider -mt-1">
              ADMIN PORTAL
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="hidden md:flex items-center relative">
            <MdSearch className="absolute left-4 text-teal-700/60" size={18} />
            <input 
              type="text" 
              placeholder="Quick search..." 
              className="bg-teal-700/5 border border-teal-700/10 rounded-full py-2.5 px-4 pl-12 text-sm w-52 transition-all duration-300 outline-none focus:bg-white"
            />
          </div>
          
          {/* Notifications */}
          <div className="relative mr-2">
            <div className="w-10 h-10 rounded-xl bg-white/80 border border-teal-700/15 flex items-center justify-center text-teal-700 cursor-pointer transition-all duration-300 hover:bg-teal-700/10">
              <MdNotificationsNone size={20} />
          ...
          <div ref={dropdownRef} className="relative">
            <div 
              className="bg-gradient-to-br from-teal-700 to-teal-800 rounded-2xl py-1.5 px-1.5 pl-4 flex items-center gap-3 shadow-lg shadow-teal-700/25 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-teal-700/35 cursor-pointer border border-white/30"
              onClick={() => setShowDropdown(!showDropdown)}
            >
              <span className="hidden md:block text-white font-semibold text-sm">
                Alexander
              </span>
              <div className="rounded-full flex items-center justify-center text-teal-700 font-bold w-9 h-9 bg-gradient-to-br from-rose-50 to-red-50 border-2 border-white shadow-md text-base">
                A
              </div>
            </div>
            ...
            {showDropdown && (
              <div 
                className="absolute top-[calc(100%+12px)] right-0 z-[1100] bg-white/90 backdrop-blur-xl border border-white/50 rounded-2xl shadow-2xl overflow-hidden p-2 min-w-[220px] animate-[slideDownHeader_0.3s_cubic-bezier(0.2,0.9,0.4,1)]"
              >
                <div className="px-3 py-2 border-b border-gray-100">
                  <div className="font-bold text-teal-700">Alexander Pierce</div>
                  <small className="text-gray-500 text-xs">admin@pixelmind.com</small>
                </div>
                
                <button
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-gray-700 font-medium hover:bg-teal-700/10 hover:text-teal-700 mt-1"
                  onClick={handleLogout}
                >
                  <MdLogout size={18} className="text-red-400" />
                  <span className="text-red-500 font-semibold">Secure Logout</span>
                </button>
              </div>
            )}
          </div>
