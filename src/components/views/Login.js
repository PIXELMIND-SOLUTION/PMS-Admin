...
const Login = ({ onLogin }) => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://crmbackend-oprd.onrender.com/api/auth/send-otp', {
        method: 'POST',
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setOtpError('');
    try {
      const response = await fetch('https://crmbackend-oprd.onrender.com/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
