  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success !== false) {
        const clientName = data.data?.name || formData.name;
        Swal.fire({
          icon: 'success',
          title: 'Client Created!',
          text: `Client "${clientName}" has been created successfully.`,
          confirmButtonColor: '#0d9488',
          timer: 2000,
      ...
      <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-800 tracking-tight">Add Client</h1>
              <p className="text-sm text-gray-500">Register new client with complete details</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3 pt-4 border-t border-gray-100">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><Loader size={17} className="spin" />Creating…</> : <><UserPlus size={17} />Add Client</>}
              </button>
            </div>
          </form>
        </div>
