...
const WelcomeCard = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const countsRes = await fetch('https://crmbackend-oprd.onrender.com/api/counts');
        const countsData = await countsRes.json();

        const staffRes = await fetch('https://crmbackend-oprd.onrender.com/api/get_all_staffs');
